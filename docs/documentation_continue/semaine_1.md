...
Lors de cette deuxième journée, j'ai découvert le système Gridfinity, un système de rangement modulaire très utilisé en impression 3D. Son principe repose sur une grille standardisée permettant de créer des bacs de différentes tailles qui s'emboîtent parfaitement dans des tiroirs ou des espaces de stockage.
...
J'ai commencé par rechercher différents modèles de bacs et de tiroirs compatibles avec Gridfinity. Ces recherches m'ont permis de découvrir de nombreuses variantes adaptées au rangement des vis, des écrous et d'autres petites pièces.
...
Le système Gridfinity étant basé sur des modules de 4,2 cm × 4,2 cm, j'ai effectué les calculs nécessaires afin de déterminer le nombre de cases pouvant être installées dans le tiroir. Le résultat obtenu est une grille de 18 modules de longueur sur 11 modules de largeur.

J'ai également installé et configuré OrcaSlicer sur mon ordinateur afin de préparer les futures impressions 3D. Cette étape m'a permis de découvrir les bases du tranchage de modèles et de la préparation des fichiers destinés aux imprimantes.
...
J'ai enfin pu découvrir une partie du parc d'imprimantes 3D du Makerspace, comprenant notamment des Bambu Lab A1 Mini, des Bambu Lab A1, une Bambu Lab P1P ainsi que plusieurs imprimantes Artillery.
